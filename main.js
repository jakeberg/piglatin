//CREATES THE FIRST TEXT AREA DIV
const destinationOne = document.querySelector("#area1");

function printResultOne(result) {
    let newElement = document.createElement("div");
    newElement.textContent = result;
    destinationOne.appendChild(newElement);
}
...
document.getElementById("firstButton").onclick = function () {
    ...
    var typedText1 = document.getElementById("input1").value;
    ...
    */
    function encodeWord(word) {
        ...
        if (vowelWordArray.indexOf("a", "e", "i", "o", "u") == 0) {
            ...
            let resultB = vowelWordArray.join("");
            return encodeConsonantWord(resultB);
        }
    }
    ...
    */
    function encodeText(text) {
        ...
        printResultOne(result.join(' '));
    }

    encodeText(typedText1)
...
//CREATES THE SECOND AREA DIV

const destinationTwo = document.querySelector("#area2");

function printResultTwo(result) {
    let newElement = document.createElement("div");
    newElement.textContent = result;
    destinationTwo.appendChild(newElement);
}

// BEGINNING OF FUNCTION TWO 

document.getElementById("secondButton").onclick = function () {

    var typedText2 = document.getElementById("input2").value;
    ...
    */
    function decodeVowelWord(word) {
        let withYay = word.split("");
        let removeYay = withYay.splice(-3, 3)
        let finalResult = withYay.join('');
        return finalResult;
    }
    ...
    */
    function decodeConsonantWord(word) {
        let withYay = word.split("");
        let indexDash = word.indexOf("-");
        let removeEnd = withYay.splice(indexDash, );
        let removeDash = removeEnd.shift();
        let removeLastTwo = removeEnd.splice(-2, 2);

        let exportWordArray = removeEnd.concat(withYay);
        let finalResult = exportWordArray.join('');
    ...
    */
    ...
    function decodeWord(word) {
        if (word.includes("-")) {
            return decodeConsonantWord(word);
        } else {
            return decodeVowelWord(word);
        }
    }
    ...
    */
    ...
    function decodeText(text) {
        let arrDecText = text.split(" ");
        let result = [];
        for (let i = 0; i < arrDecText.length; i++) {
            result.push(decodeWord(arrDecText[i]));
        }
        /* this is the same as lines 179 to 182 above 
        let result = arrDecText.map(function(word) {
            return decodeWord(word);
        });
        */
        printResultTwo(result.join(' '));
    }



    decodeText(typedText2)
